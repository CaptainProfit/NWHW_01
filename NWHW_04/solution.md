# Лабораторная работа. Настройка IPv6-адресов на сетевых устройствах 
## цели
- Настройка топологии и конфигурация основных параметров маршрутизатора и коммутатора
- Ручная настройка IPv6-адресов
- Проверка сквозного соединения

## Часть 1. Настройка топологии и конфигурация основных параметров маршрутизатора и коммутатора
- собрал схему
- назначил имена хостов
![network topology](01.png)

## Часть 2. Ручная настройка IPv6-адресов
таблица адресации из задания

|            |           |                    |                       |                |                   |
| ---------- | --------- | ------------------ | --------------------- | -------------- | ----------------- |
| Устройство | Интерфейс | IPv6-адрес         | Link local IPv6-адрес | Длина префикса | Шлюз по умолчанию |
| R1         | G0/0/0    | 2001:db8:acad:a::1 | fe80::1               | 64             | —                 |
| R1         | G0/0/1    | 2001:db8:acad:1::1 | fe80::1               | 64             | —                 |
| S1         | VLAN 1    | 2001:db8:acad:1::b | fe80::b               | 64             | —                 |
| PC-A       | NIC       | 2001:db8:acad:1::3 | SLACC                 | 64             | fe80::1           |
| PC-B       | NIC       | 2001:db8:acad:a::3 | SLACC                 | 64             | fe80::1           |

### Шаг 1. Назначьте IPv6-адреса интерфейсам Ethernet на R1.
```
enable
config term
interf gi 0/0/0
ipv6 add fe80::1 link
ipv6 add 2001:db8:acad:a::1/64
no shut
exit
interf gi 0/0/1
ipv6 add fe80::1 link
ipv6 add 2001:db8:acad:1::1/64
no shut
exit

```
теперь адреса назначены, проверим с помощью `show running-config`

![ipv6 on router](02.png)

так же поднялись сети на роутере.
с помощью `show ipv6 interface brief` ещё нагляднее
![ipv6 on router](03.png)

если сразу линк локал адрес не назначить - он сгенерируется на основе MAC-адреса интерфейса (см EUI-64)
![ipv6 default linklocal](04.png)

![mac-addr](05.png)

> Какие группы многоадресной рассылки назначены интерфейсу G0/0?
> - FF00::1 - для всех узлов
> - ~~FF00::2 - для всех коммутаторов~~. Отсутствует, так как я не включал unicast-routing 
> - ~~прочие адреса FF00::/8~~
### Шаг 2. Активируйте IPv6-маршрутизацию на R1.
пока что адрес хосту не назначается

![no ipv6 addess on host](06.png)

на роутере выполним `ipv6 unicast-routing` и поставим автматическое определение IPv6-адреса на хосте и
![auto ipv6 addess on host](07.png)
> **Почему PC-B получил глобальный префикс маршрутизации и идентификатор подсети, которые вы настроили на R1?** Так как я не установил адрес статически - он определяется хостом самостоятельно. По протоколу ICMPv6 хост отправил сообщения RS, а маршрутизатор отправил в ответ сообщение RA cx префиксом маршрутизации и идентификатором подсети и использует их для генерации своего уникального адреса по MAC-адресу. Это механизм автоматического назначнеия своего адреса SLAAC.
### Шаг 3. Назначьте IPv6-адреса интерфейсу управления (SVI) на S1.
**SVI** - Switch virtual interface
**SDM** - Switch Database Manager
на коммутаторе сначала нужно настроить sdm:
> SDM коммутатора 2960 по умолчанию не поддерживает ipv6 в коммутаторе может понадобиться `sdm prefer dual-ipv4-and-ipv6 default` или `lanbase-routing` для включения IPv6-адресации.
смотрим
`S1# show sdm prefer`
> на что смотреть то в итоге: должно быть `The current template is "dual-ipv4-and-ipv6 default" template.`
устанавливаем
```
S1# configure terminal
S1(config)# sdm prefer dual-ipv4-and-ipv6 default
S1(config)# end
S1# copy ru st
S1# reload
```
Теперь настройка svi
```
enable
config term
interf vlan 1
ipv6 add fe80::b link
ipv6 add 2001:db8:acad:1::b/64
no shut
exit
exit
```

`show ipv6 interface brief` - результат
![switcher ipv6 table](08.png)
### Шаг 4. Назначьте компьютерам статические IPv6-адреса.
автоматические адреса уже настроились в шаге 1, теперь задам статические
![pc-A static](09.png)
![pc-B static](10.png)

## Часть 3. Проверка сквозного подключения
В первый раз связность не появилась и пришлось дополнительные телодвижения проводить. Повторил лабораторную с нуля более аккуратно - всё поехало сразу.
> ~~связности нет,~~ 
~~перепутал местами интерфейсы роутера, меняю местами~~
~~связности нет, делаю на роутере~~
~~`ipv6 unicast-routing`~~
~~связности нет, делаю на интерфейсах роутера~~
~~`ipv6 enable`~~
~~связности нет, всё перепроверил, сохранил настройки, перезагрузил коммутатор и маршрутизатор~~
~~связность появилась~~
### С хоста PC-A
![ping A](11.png)
![tracert A](12.png)
### С хоста PC-B
![ping B](13.png)
# Вопросы для повторения
1. Почему обоим интерфейсам Ethernet на R1 можно назначить один и тот же локальный адрес канала — FE80::1? 
- Потому что **этот адрес не маршрутизуется** и актуален только в рамках одного канала (или канального сегмента? то есть в рамках подсети? лла не маршрутизируется, как приватный, а коммутируется?)
1. Какой идентификатор подсети в индивидуальном IPv6-адресе 2001:db8:acad::aaaa:1234/64?  
- если считать длину префикса глобальной маршрутизации 48 а длину идентификатора подсети 16, то:
	- префикс глобальной маршрутизации будет 2001:db8:acad:: / 48
	- идентификатор подсети будет 0000